...
# Enhanced user database with file persistence
...
# If no users exist, create default demo user
if not users_db:
    users_db = {
        'demo@healthcare.com': {
            'password': 'demo123',
...
def create_user_session(email):
    """Create a user session with enhanced data"""
    user = users_db[email]
...
def login_required(f):
    def decorated_function(*args, **kwargs):
        ...
        if user_email and user_email not in users_db:
            clear_user_session()
            flash('Your account no longer exists. Please register again.', 'error')
            return redirect(url_for('login'))
